...
pub trait OrbitRepo {
  ...
  async fn fetch_orbit_id_from_fediverse_id(&self, shortcode: &str) -> Option<Uuid>;
...
impl OrbitRepo for DbOrbitRepo {
  ...
  async fn fetch_orbit_id_from_fediverse_id(&self, fediverse_id: &str) -> Option<Uuid> {
    let db = match self.db.get().await.map_err(map_db_err) {
      Ok(db) => db,
      Err(_) => return None,
    };
    let row = match db
      .query_opt("SELECT orbit_id FROM orbits WHERE fediverse_id = $1", &[&fediverse_id])
      .await
      .map_err(map_db_err)
    {
      Ok(row) => row,
