...
pub async fn federate_user_actor_from_webfinger(
  ...
  let result: Result<Response, reqwest::Error> = retry(BACKOFF_POLICY.clone(), || async {
    Ok(HTTP_CLIENT.get(&uri).send().await?)
  })
  .await;

  let result: Result<Option<WebfingerRecord>, reqwest::Error> = match result {
    Ok(response) => {
